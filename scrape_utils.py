...
def get_acronym(acronym) -> str:
    ...
    finally: return f'URL:`{url}`{chr(0x0a)*2}RESPONSE:{chr(0x0a)}```{acro_list[:MAX_RETURN_VALUE_LENGTH] if len(acro_list) > MAX_RETURN_VALUE_LENGTH else acro_list}```'
...
def get_definition(search_term) -> str:
    ...
    finally: return f'URL:`{url}`{chr(0x0a)*2}RESPONSE:{chr(0x0a)}```{short_wiki.text[:MAX_RETURN_VALUE_LENGTH] if len(short_wiki.text)>MAX_RETURN_VALUE_LENGTH else short_wiki.text}```'
