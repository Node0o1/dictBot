...
def get_definition(search_term) -> str:
    ...
    finally: return f'URL:{url}{chr(0x0a)*2}RESPONSE:{chr(0x0a)}```{short_wiki[:(MAX_RETURN_VALUE_LENGTH - (24 + len(url)))] if (len(short_wiki) + 24 + len(url)) > MAX_RETURN_VALUE_LENGTH else short_wiki}```'
